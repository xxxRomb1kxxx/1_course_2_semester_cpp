...
void create_bin_file() {
    std::ofstream out("laba_govno_1.bin", std::ios::binary);
    uint8_t buf[6];
    buf[0] = 5;
    for (int i = 0; i < 5; i++)
    {
    ...
    if (out.is_open()) {
        ...
        std::cout << "File path: " << fs::absolute("laba_govno_1.bin") << std::endl;
    }
    else {
...
void create_binf_file() {
    std::ofstream out("example.binf", std::ios::binary); 
    ...
    if (out.is_open()) {
        std::cout << "File created successfully" << std::endl;
        std::cout << "File path: " << fs::absolute("example.binf") << std::endl;
    }
    else {
        std::cerr << "Failed to create file" << std::endl;
    }
